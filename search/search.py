...
def depthFirstSearch(problem):
    ...
    print "Is the start a goal?", problem.isGoalState(problem.getStartState())
    print "Start's successors:", problem.getSuccessors(problem.getStartState())
    print problem.getSuccessors(problem.getStartState())[0]
    ...
    while not st.isEmpty():
	    ...
	    break
        if visited[curState] is 0:
            visited[curState] = 1
            for successor in problem.getSuccessors(curState):
                nextState = successor[0]
                if visited[nextState] is 0:
		    '''
                    print "parent of ",nextState,"is ",curState
                    '''
                    parent[nextState]=curState
                    toState[nextState]=(str(successor[1]))
                    st.push(nextState)
    state=curState
    while state is not problem.getStartState():
        path.insert(0, toState[state])
        state=parent[state]
    ...
    while not pq.isEmpty():
        ...
        if visited[curState] is 0:
		    ...
		    '''
                    ...
                    parent[nextState]=curState
                    toState[nextState]=(str(successor[1]))
                    pq.push(nextState, heuristic(curState, problem, '')+successor[2])
    state=curState
    while state is not problem.getStartState():
        path.insert(0, toState[state])
        state=parent[state]
