    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                weighted[p] += emissionModel[trueDistance]     
            ...
            if weighted.totalCount()==0 :
               self.initializeUniformly(gameState)
                ...
                self.particles = []
                for i in range(self.numParticles) :
                    self.particles.append(util.sample(weighted))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        combinatorics = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(combinatorics)
        self.particles = [combinatorics[i % len(combinatorics)] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        weighted_particals = util.Counter()
        for p in self.particles:
            weighted = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] is not None:
                    trueDistance = util.manhattanDistance(pacmanPosition, p[i])
                    weighted *= emissionModels[i][trueDistance]
            weighted_particals[p] += weighted
            
        weighted_particals.normalize()
        
        if weighted_particals.totalCount()==0:
            self.initializeParticles()
        else:
            self.particles = [util.sample(weighted_particals) for i in range(self.numParticles)]
