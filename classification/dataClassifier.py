...
def bfs (start, visited, datum):
    ...
    while not q.isEmpty():
        ...
        for dir in [(-1,0),(1,0),(0,-1),(0,1)]:
            x = cur[0]+dir[0]
            y = cur[1]+dir[1]
            if 0<=x and x<DIGIT_DATUM_WIDTH and 0<=y and y<DIGIT_DATUM_HEIGHT:
                if datum.getPixel(x, y) == 0 and (x,y) not in visited:
                    visited.add((x,y))
                    q.push((x,y))
...
def enhancedPacmanFeatures(state, action):
    ...
    minDist = 9999.0
    totalDist = 0
    p = successor.getPacmanPosition()
    for gp in successor.getGhostPositions():
        minDist = min(minDist, util.manhattanDistance(p, gp))
        #totalDist += util.manhattanDistance(p, gp)
    features['minDist'] = minDist
    #features['totalDist'] = totalDist
    
    
    shortestFood = 9999.0
    for f in successor.getFood():
        shortestFood = min(shortestFood, util.manhattanDistance(p,f))
        
    features['shortestFood'] = shortestFood

    
    #closestCap = 9999
    #for c in successor.getCapsules():
    #    closestCap = min(closestCap, util.manhattanDistance(p, c))
    
    #features["closestCap"] = closestCap
    
    
    features["score"] = state.getScore() / 25.0
