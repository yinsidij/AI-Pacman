...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        if (state not in self.qValueHash):
            return 0.0
        if (action not in self.qValueHash[state]):
            return 0.0
        return self.qValueHash[state][action]
    ...
    def update(self, state, action, nextState, reward):
        ...
        "*** YOUR CODE HERE ***"
        if (state not in self.qValueHash):
            self.qValueHash[state] = dict()
        if (action not in self.qValueHash[state]):
            self.qValueHash[state][action] = dict()
            self.qValueHash[state][action] = 0.0
            
        self.qValueHash[state][action] = (1-self.alpha)*self.qValueHash[state][action] + \
